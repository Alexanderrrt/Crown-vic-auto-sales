...
export async function getInventory(): Promise<Vehicle[]> {
  ...
  const { data, error } = await client
    .from("vehicles")
    .select("*, vehicle_media(url, sort_order)")
    .neq("status", "draft")
  ...
  return data.map((row) => {
    const media = Array.isArray(row.vehicle_media)
      ...
      : undefined;
    return toVehicle({ ...row, media, image: row.image || media?.[0] });
  });
}
...
export async function getVehicleBySlug(slug: string): Promise<Vehicle | null> {
  ...
  const { data, error } = await client.from("vehicles").select("*, vehicle_media(url, sort_order)").eq("slug", slug).maybeSingle();
  ...
  return toVehicle({ ...data, media, image: data.image || media?.[0] });
}
